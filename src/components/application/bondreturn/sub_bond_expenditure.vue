...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/finance/sub_bond_expenditurelist", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/finance/sub_bond_expendituredel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/finance/sub_bond_expenditure").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
