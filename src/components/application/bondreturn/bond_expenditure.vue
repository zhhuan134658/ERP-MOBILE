...
export default {
  ...
  data() {
    return {
      valname: "services_bond_expenditure",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/finance/services_bond_expenditurelist", {
          current_page: this.current_page,
          num: "10",

          starttime: this.starttime,
          stoptime: this.stoptime,
          status: this.shenpival,
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/finance/services_bond_expendituredel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/finance/services_bond_expenditure").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
