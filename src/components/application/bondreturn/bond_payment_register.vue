...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/finance/bond_payment_registerlist", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/finance/bond_payment_registerdel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/finance/bond_payment_register").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
