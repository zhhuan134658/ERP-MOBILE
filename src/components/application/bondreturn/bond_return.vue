...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/finance/services_bond_returnlist", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/finance/services_bond_returndel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/finance/services_bond_return").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
