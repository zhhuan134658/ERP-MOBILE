...
export default {
  ...
  data() {
    return {
      valname: "machinery_fee_register",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/machineryFeeRegisterList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/machineryFeeRegisterDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/machineryFeeRegister").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
