...
export default {
  ...
  data() {
    return {
      valname: "safety_inspection_record",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/safetyInspectionRecordList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/safetyInspectionRecordDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/safetyInspectionRecord").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
