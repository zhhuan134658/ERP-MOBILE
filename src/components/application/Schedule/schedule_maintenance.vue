...
export default {
  ...
  data() {
    return {
      valname: "construction_list_maintenance",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/constructionListMaintenanceList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/constructionListMaintenanceDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/constructionListMaintenance").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
