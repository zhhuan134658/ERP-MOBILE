...
export default {
  ...
  data() {
    return {
      valname: "income_contract_change_visa",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/incomeContractChangeVisaList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/incomeContractChangeVisaDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/incomeContractChangeVisa").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
