...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/materialContractList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/materialContractDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      // this.$router.push({ path: '/material_contractcreat' });
      const _this = this;
      console.log(_this.datainfo);

      _this.$axios.post("/project/materialContract").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
