...
export default {
  ...
  data() {
    return {
      valname: "material_requirement_plan",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/materialRequirementPlanList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/materialRequirementPlanDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      // this.$router.push({ path: '/demand_plancreat' });
      const _this = this;
      console.log(_this.datainfo);

      _this.$axios.post("/project/materialRequirementPlan").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
