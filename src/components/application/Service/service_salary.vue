...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/sporadicServiceListList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/sporadicServiceListListDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/sporadicServiceList").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
