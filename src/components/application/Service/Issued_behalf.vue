  ...
  methods: {
    setTitle() {
      ...
      dd.biz.navigation.setTitle({
        title: _this.$route.query.itemdata.name, //控制标题文本，空字符串表示显示默认文本
    ...
    getInfo() {
      this.$axios
        .post("/project/issuedClassWagesList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/issuedClassWagesDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/issuedClassWages").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
