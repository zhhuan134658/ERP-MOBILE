...
export default {
  ...
  data() {
    return {
      valname: "service_warranty_settlement",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/projecttwo/serviceWarrantySettlementList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/projecttwo/serviceWarrantySettlementDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/projecttwo/serviceWarrantySettlement").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
