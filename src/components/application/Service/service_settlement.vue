...
export default {
  ...
  data() {
    return {
      valname: "service_completion_settlement",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/serviceCompletionSettlementList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/serviceCompletionSettlementDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/serviceCompletionSettlement").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
