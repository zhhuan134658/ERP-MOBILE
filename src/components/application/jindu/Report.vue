  ...
  <div id="creatschedule">
    ...
    >
      ...
      <van-field
        ...
        input-align="right"
        v-model="eventFrom.liname"
        name="上报里程碑"
        label="上报里程碑"
        placeholder="请填写上报里程碑"
        maxlength="10"
        :rules="[{ required: true }]"
      />
      ...
      <div class="imgupload" style="margin-bottom: 0.33rem">
        ...
        <div class="upList">
          <div
            class="xiangmu"
            v-for="(item, index) in eventFrom.biz_data"
            :key="index"
          >
...
export default {
  ...
  methods: {
    ...
    imguploadAtta(newspaceid) {
      ...
      dd.ready(function () {
        dd.biz.util.uploadAttachment({
          ...
          onSuccess: function (result) {
            _this.eventFrom.biz_data = _this.deunique(
              _this.eventFrom.biz_data.concat(result.data)
            );
          },
          onFail: function (err) {},
        });
    ...
    //删除
    deletefile(item) {
      this.eventFrom.biz_data.splice(this.eventFrom.biz_data.indexOf(item), 1);
    },
    deunique(arr) {
      const res = new Map();
      return arr.filter(
    ...
    onSubmit() {
      //否 “1” 是 “2”
      this.$axios.post("/task/reportprogress", this.eventFrom).then((res) => {
        if (res.data.code == 1) {
          Toast(res.data.msg);
