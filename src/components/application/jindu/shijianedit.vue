  ...
  <div id="creatschedule">
    ...
    >
      ...
      <van-field
        ...
        input-align="right"
        maxlength="50"
        v-model="eventFrom.jdusers[0].name"
        name="负责人"
        label="负责人"
        placeholder="点击选择负责人"
        @click="addren"
  ...
  methods: {
    ...
    gettime(value) {
      ...
      if (this.valtime == "1") {
        ...
        this.enddata = value;
        let newa = new Date(this.eventFrom.starttime);
        console.log(newa);
    ...
    onSubmit() {
      //否 “1” 是 “2”
      this.eventFrom.type = "update";
      this.eventFrom.user_id = this.$store.state.userData.userid;
      this.eventFrom.userid = this.$store.state.userData.userid;
      this.eventFrom.jid = this.$route.query.itemdata.id;

      this.$axios.post("/task/UpdateJinDu", this.eventFrom).then((res) => {
        if (res.data.code == 1) {
          Toast(res.data.msg);
    ...
    getinfo() {
      this.$axios
        ...
        .then((res) => {
          this.eventFrom = res.data.content;
          this.cc_name = this.eventFrom.cc_userid.map((item) => {
            return item.name;
          });
          if (this.eventFrom.jdstatus == 3) {
            this.btnshow = false;
          }
        });
    },
