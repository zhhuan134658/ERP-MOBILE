...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      //否 “1” 是 “2”
      this.$axios.post("/finance/schedule", this.addForm).then(res => {
        if (res.data.code == 1) {
          Toast(res.data.msg);
        ...
        dd.biz.contact.complexPicker({
          ...
          onSuccess: function(result) {
            if (val == 1) {
              ...
              //   that.xiezuozong = result.users[0].name;
    ...
    geteditinfo() {
      this.$axios
        .post("/finance/feedback", {
          id: this.$store.state.userData.lcxminfo.id,
          type: 1
        })
        .then(res => {
  ...
  watch: {
    checkbox: {
      handler(newValue) {
        ...
        console.log("78978", this.addForm);
      }
    }
  },
  created() {
    ...
    this.addForm.project_name = this.$store.state.userData.lcxminfo.name;
