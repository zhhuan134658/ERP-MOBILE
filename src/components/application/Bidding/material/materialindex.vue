...
export default {
  ...
  data() {
    return {
      valname: "large_material_bidding_plan",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/largeMaterialBiddingPlanList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/largeMaterialBiddingPlanDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/largeMaterialBiddingPlan").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
