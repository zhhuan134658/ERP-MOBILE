...
export default {
  ...
  data() {
    return {
      valname: "subcontract_bidding_plan",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/subcontractBiddingPlanList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/subcontractBiddingPlanDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/subcontractBiddingPlan").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
