...
export default {
  ...
  data() {
    return {
      valname: "bidding_offer_contrast",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/biddingOfferContrastList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/biddingOfferContrastDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/biddingOfferContrast").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
