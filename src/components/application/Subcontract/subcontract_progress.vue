...
export default {
  ...
  data() {
    return {
      valname: "sub_progresspay_declaration",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/subProgresspayDeclarationList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/subProgresspayDeclarationDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/subProgresspayDeclaration").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
