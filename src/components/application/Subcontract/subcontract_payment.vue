    ...
    <van-list v-model="loading" :finished="finished" @load="onLoad">
      <div class="renwucontent" v-for="(item, index) in FilesList" :key="index">
        <div style="padding: 0.5rem 1rem" @click="lookinfo(item)">
          ...
          <div class="btem">
            <div class="btem1">付款编号:</div>
            <div class="btem2">{{ item.number }}</div>
          </div>
          <div class="btem">
            <div class="btem1">项目名称:</div>
            <div class="btem2">{{ item.project_name }}</div>
          </div>
          <div class="btem">
            <div class="btem1">分包单位:</div>
            <div class="btem2">{{ item.sub_unit }}</div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      valname: "sub_payment_application",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/subPaymentApplicationList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/subPaymentApplicationDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/subPaymentApplication").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
