...
export default {
  ...
  data() {
    return {
      valname: "sub_completion_settlement",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/project/subCompletionSettlementList", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/project/subCompletionSettlementDel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/project/subCompletionSettlement").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
