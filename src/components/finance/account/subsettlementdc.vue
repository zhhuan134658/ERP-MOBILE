...
export default {
  ...
  data() {
    return {
      valname: "sub_warranty_settlement",
      waringdata: false,
      // 筛选
      typename: "",
      shenpitype: "",
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/finance/subsettlementlist", {
    ...
    creatin() {
      ...
      _this.$axios.post("/finance/subsettlement").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
