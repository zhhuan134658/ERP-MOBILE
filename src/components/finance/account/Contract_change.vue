...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.$axios
        .post("/finance/Laborchangevisalist", {
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("/finance/Laborchangevisadel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/finance/Laborchangevisa").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
