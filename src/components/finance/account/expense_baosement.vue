...
export default {
  ...
  methods: {
    ...
    onLoad() {
      this.$axios
        .post("/journal/reimbursementlist", {
          current_page: this.current_page,
          num: "10",
          starttime: this.starttime,
          stoptime: this.stoptime,
          bxstatus: this.shenpival,

          bxname: this.Searchname,
        })
        .then((res) => {
          if (res.data.code == 1) {
            this.loading = false;
    ...
    deleteitem(item) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          this.$axios
            .post("journal/reimbursementlistdel", {
              id: [item.id],
            })
            .then((res) => {
              if (res.data.code == 1) {
    ...
    creatin() {
      ...
      _this.$axios.post("/finance/moneybaoxiao").then((res) => {
        if (res.data.code == 1) {
          _this.openurl =
